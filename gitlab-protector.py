...
class ProtectionManager:
    ...
    def apply_tag_protection(
        ...
        try:
            ...
            Logger.debug(
                f"protecting tag: {name} (merge={merge_level}, push={push_level})"
            )

            protection = {
                "name": name,
                "create_access_level": mal,  # Who can create the tag
                "allowed_to_create": [],  # Additional users/groups (empty for now)
            }
...
class GitLabProtector:
    ...
    def _collect_projects(self) -> None:
        ...
        try:
            ...
            root_group = self.gitlab_api.groups.get(
                self.config.namespace, lazy=True, include_subgroups=True
            )
    ...
    def _process_subgroups(self) -> None:
        ...
        try:
            ...
            descendant_groups = self.gitlab_api.groups.list(
                all=True, owned=True, top_level_only=False, search=self.config.namespace
            )

            for group in descendant_groups:
                # Skip if excluded
                group_path = getattr(group, "full_path", "")
                if self._is_excluded(group_path):
                    Logger.warn(f"excluding: {group_path}")
                    continue

                # Get full group object and add its projects
                group_id = getattr(group, "id", None)
                if group_id:
                    full_group = self.gitlab_api.groups.get(group_id, lazy=True)
